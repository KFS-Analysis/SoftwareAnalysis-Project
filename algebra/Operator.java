...
public class Operator {
	/**
	 * Constructs an operator from a given method. It is assumed that the method has
	 * only generic type variables as parameter and result types, and that for each
	 * such type variable <code>Xxx</code> there is a corresponding signature
	 * <code>XxxSignature</code>.
	 * 
	 * @param method the method to be converted into an operator
	 * @throws IllegalArgumentException if the method parameter or return types are
	 *                                  not type variables.
	 */
		...
		for (int i = 0; i < this.arity; i++) {
			...
			if (this.setOperator) {
				if (((ParameterizedType) type).getRawType() != List.class) {
					throw new IllegalArgumentException("Method '%s' does not represent collection operator");
				}
	...
	private Sort toSort(Type type) throws IllegalArgumentException {
		...
		if (result == null) {
			throw new IllegalArgumentException(String.format("Type '%s' is not an existing sort", typeName));
		}
		return result;
	}
	...
	/**
	 * Returns the number of parameters of this operator. For a collection-based
	 * operator, the arity is 1.
	 */
	public int getArity() {
		return this.arity;
	}
	...
	/**
	 * Returns the in- or prefix symbol of this operator, or {@code null} if it has
	 * none.
	 */
	...
	/**
	 * Returns the description in the {@link ToolTipHeader} annotation of the
	 * method.
	 */
	...
	/**
	 * Indicates if there are more operators with the same name and parameter count.
	 */
	...
	/**
	 * Constructs and returns a new composite term consisting of this operator
	 * applied to a sequence of arguments.
	 */
	...
	/**
	 * Returns the method from a given signature class with a given name. This
	 * method is supposed to implement an operator, and should therefore be declared
	 * exactly once, as a public abstract method.
	 */
	private static Method getOperatorMethod(Class<?> sigClass, java.lang.String name) {
		...
		java.lang.String sigName = className.substring(0, className.indexOf("Signature")).toLowerCase();
		Method[] methods = sigClass.getDeclaredMethods();
		for (Method method : methods) {
			if (method.getName().equals(name)) {
				if (result != null) {
					throw new IllegalArgumentException(
							java.lang.String.format("Operator overloading for '%s:%s' not allowed", sigName, name));
				}
				result = method;
			}
		}
		...
		if (!Modifier.isAbstract(result.getModifiers())) {
			throw new IllegalArgumentException(
					java.lang.String.format("Method for operator '%s:%s' should be abstract", sigName, name));
		}
		if (!Modifier.isPublic(result.getModifiers())) {
			throw new IllegalArgumentException(
					java.lang.String.format("Method for operator '%s:%s' should be public", sigName, name));
		}
		return result;
	}
		...
		if (ops.isEmpty()) {
			for (Sort sort : Sort.values()) {
				for (OpValue opValue : sort.getOpValues()) {
					ops.addAll((Collection<? extends Operator>) opValue.getOperator());
				}
			}
		}
		return ops;
	...
	/**
	 * Returns a map from operator symbols and names to operators with that
	 * symbol/name.
	 */
	...
	/**
	 * Mapping from operator names and symbols to lists of operators with that
	 * symbol.
	 */
